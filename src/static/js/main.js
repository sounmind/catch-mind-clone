...
"use strict";var $body=document.querySelector("body"),nickname=localStorage.getItem("nickname"),LOGGED_OUT="loggedOut",LOGGED_IN="loggedIn",NICKNAME="nickname",$loginForm=document.querySelector("#jsLogin"),logIn=function(e){window.socket=io("/"),window.socket.emit(window.events.setNickname,{nickname:e})};nickname?($body.className=LOGGED_IN,logIn(nickname)):$body.className=LOGGED_OUT;var handleFormSubmit=function(e){e.preventDefault();var n=$loginForm.querySelector("input"),o=n.value;n.value="",localStorage.setItem(NICKNAME,o),$body.className=LOGGED_IN,logIn(o)};$loginForm&&$loginForm.addEventListener("submit",handleFormSubmit);
